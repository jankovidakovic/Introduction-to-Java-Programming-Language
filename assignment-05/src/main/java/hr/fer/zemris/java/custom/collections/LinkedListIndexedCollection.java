...
public class LinkedListIndexedCollection<E> implements List<E> {
	...
	private static class ListNode<V> {
		...
		//private variables
		private final V value;		//value of current node
		private ListNode<V> previous;	//previous node in the list
		private ListNode<V> next;		//next node in the list
		 ...
		 */
		private ListNode(V value, ListNode<V> previous, ListNode<V> next) {
			this.value = value;
	 ...
	 * If new node is successfully added, then this operation counts as a
	...
	public boolean contains(Object value) {
		for(ListNode<E> it = first; it != null; it = it.next) { //iterate over the list
	 ...
	 * Occurrence is determined by the <code>equals</code> method.
	...
	public void remove(int index) {
		...
		ListNode<E> nextNode = it.next; //next node
		ListNode<E> previousNode = it.previous; //previous node
	...
	public E[] toArray() {
		E[] newArray = (E[])new Object[size];	//allocating memory
		ListNode<E> it;	//iterator node
		int i;			//index
	...
	@Override
	public void clear() {
		for (ListNode<E> it = first; it != null; it = it.next) {
	...
	public E get(int index) {
		...
		ListNode<E> it;	//starting point of list traversal
	 ...
	 * successfully, it counts as a modification of the collection, which means
	...
	public void insert(E value, int position) {
		...
		ListNode<E> newNode = new ListNode<E>(value, null, null); //new node to insert
		
		if (isEmpty()) {	//special case, list is empty
			first = last = newNode;	//previous and next can stay null
		} else {
			ListNode<E> it;	//for traversing, will point to node at given position
	...
	public int indexOf(Object value) {
		ListNode<E> it;	//helper node for list traversal
	...
	private class LinkedListElementsGetter implements ElementsGetter<E> {
		...
		private ListNode<E> nextElement;	//next retrievable element
		
		/**
		 * Default constructor. Sets up the <code>ElementsGetter</code>
		 * for retrieval of the first node of the list, and remembers
		 * the current modification count of the list, so it can track
		...
		/**
		 * Checks whether list contains any elements that have not
