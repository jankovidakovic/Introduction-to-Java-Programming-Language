 ...
 * of them are checked exceptions. Furthermore, all exceptions are thorougly
...
public class ArrayIndexedCollection implements List {
	...
	public ArrayIndexedCollection(int initialCapacity) {
		if (initialCapacity < 1) { 		//capacity can't be smaller than 1
			...
			final int capacity = initialCapacity;		//initial capacity
			elements = new Object[capacity];	//memory allocation
	 ...
	 * If new value is added succesfully, then the calling of this
	 ...
	 * this method has been succesfully executed.
	...
	public void clear() {
		for (int i = 0; i < size; i++) {
			elements[i] = null;		// so that the memory can be claned
		}
	 ...
	 * will be realocated and its capacity will double.
	 * Succesful insertion of new value counts as a modification of the instance
	...
	public void insert(Object value, int position) {
		if (position < 0 || position > size) { //invalid position
			...
			for (int i = size-1; i >= position; i--) {	//elements at greater index
				elements[i+1] = elements[i];	//are shifted by one towards the end
			}
	...
	/**
	 * Searches collection and finds the first occurence of the given element.
	 ...
	 * @return index of first occurence of the given element, or -1 if collection
	 * 			does not contain the element.
	 */
	public int indexOf(Object value) {
		int indexOfFirstOccurence = -1; //return value, initialized for null
		
		if (value != null) {			//null is guaranteed not to be found
			for (int i = 0; i < size; i++) {//finding first occurrence
				if (elements[i].equals(value)) {
					indexOfFirstOccurence = i;	//rememeber the index
					break;						//found it
				}
			}
		}
	 ...
	 * Succesfull removal of element counts as a modification, and disables 
	...
	public boolean contains(Object value) {
		if (indexOf(value) == -1) { //value is not found 
			return false;
		} else {					//value is found 
			return true;	
		}
	...
	/**
	 * Removes first occurence of a given object from the collection.
	 * All elements with index greater than the one of removed element are
	 * shifted one place towards the beginning. Occurence of given object
	 * is determined by <code>equals</code> method.
	 * Succesful removal of object counts as a modification, and consequently
	 ...
	 * 				instances.
	 * @return <code>true</code> if collection contains given object
	 * 		(and consequently, its first occurence is removed),
	 * 		<code>false</code> otherwise
	 */
	public boolean remove(Object value) {
		...
		if (index == -1) {	//collection doesn't contain given element
			...
			remove(index);	//removes the first occurence of element
							//also modifies modificaionCount
			return true;
		}
	}
	...
	public Object[] toArray() {
		...
		for (int i = 0; i < size; i++) {
			newArray[i] = elements[i];	//copying all elements to the new array
		}
		return newArray;
	...
	private void resize(int targetCapacity) {
		...
		for (int i = 0; i < size; i++) {
			temporaryArray[i] = elements[i];	//copy to temporary array
		}
	...
	private class ArrayElementsGetter implements ElementsGetter {
		...
		/**
		 * DISCLAIMER - homework instructions say that this class should be
		 * implemented as a static inner class, and also that it should
		 * operate upon the collection by accessing it's inner container,
		 * instead of creating copies of elements. Those two things seem to
		 * be mutually exclusive, because even if we make this class static 
		 * and pass a reference to the instance of the collection in the 
		 * constructor, we will still not be able to use the inner conatiner 
		 * directly since it is a private field. Also, if the inner class was 
		 * made static, then every instance of elements getter created for the 
		 * same collection should be provided with the same reference. By 
		 * making the class non-static, that reference is provided implicitly. 
		 * Therefore, I have implemented this class as non-static.
		 */
		 ...
		 * elements getter is acive, then any further usage of that instance will
		 * throw an exception.
		 */
		...
		/**
		 * Checks wherher collection contains any elements that have not
		 ...
		 * deleted).
		 * @return next unretrieved element of the collection.
		 * 
		 */
		@Override
